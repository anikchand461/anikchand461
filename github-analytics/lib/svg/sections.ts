...
export function stats(repos: Res<GhRepo[]>, contrib: Res<ContribData>, counts: Res<Counts>, bytes: Res<LanguageBytes>): Block {
  ...
  rows.forEach(([label, value, color], i) => {
    const y = 60 + i * 42;
    body += circle(6, y - 5, 5.5, color) + text(22, y, label, { size: 12 }) + text(330, y, value, { size: 14, weight: 700, fill: color, anchor: 'end' });
  });

  // ring: contribution frequency
  body += ring(430, 118, 56, s.frequency, P.green);
  body += text(430, 126, `${s.frequency.toFixed(0)}%`, { size: 26, weight: 700, fill: P.green, anchor: 'middle' });
  body += text(430, 148, 'ACTIVE DAYS', { size: 6.5, fill: C.muted, anchor: 'middle', spacing: 0.3 });

  // languages
  const langs = repos.ok ? languageShares(bytes.ok ? bytes.v : null, repos.v).rows.slice(0, 6) : [];
  const lx = 540;
  const lw = INNER - lx;
  body += text(lx, 16, 'Most Used Languages', { size: 13, weight: 700, fill: P.cyan });
  body += rect(lx, 38, lw, 14, C.card2, 7);
  let cx = lx;
  langs.forEach((l) => {
    const w = (l.percent / 100) * lw;
    body += rect(cx, 38, w, 14, languageColor(l.name), 0);
    cx += w;
  });
  langs.forEach((l, i) => {
    const x = lx;
    const y = 92 + i * 30;
    body += circle(x + 6, y - 6, 6, languageColor(l.name)) + text(x + 20, y, trunc(l.name, 16), { size: 12 }) + text(x + lw, y, `${l.percent.toFixed(1)}%`, { size: 12, anchor: 'end', fill: C.muted });
  });
  if (!langs.length) body += text(lx, 92, 'No language data.', { size: 11, fill: C.muted });

  // big three
  const y0 = 290;
  body += line(0, y0 - 12, INNER, y0 - 12);
  body += line(INNER / 3, y0, INNER / 3, y0 + 210) + line((INNER * 2) / 3, y0, (INNER * 2) / 3, y0 + 210);
  const c1 = INNER / 6;
  const c2 = INNER / 2;
  const c3 = (INNER * 5) / 6;
  body +=
    text(c1, y0 + 88, fmt(s.total), { size: 40, weight: 700, fill: P.blue, anchor: 'middle' }) +
    text(c1, y0 + 136, 'Total Contributions', { size: 14, anchor: 'middle' }) +
    text(c1, y0 + 166, `${contrib.v.days[0]?.date ?? ''} → present`, { size: 10.5, fill: C.muted, anchor: 'middle' });
  body +=
    ring(c2, y0 + 62, 50, (s.currentStreak / Math.max(s.longestStreak, 1)) * 100, P.orange) +
    text(c2, y0 + 78, String(s.currentStreak), { size: 30, weight: 700, fill: P.orange, anchor: 'middle' }) +
    text(c2, y0 + 158, 'Current Streak', { size: 14, weight: 700, fill: P.orange, anchor: 'middle' }) +
    text(c2, y0 + 188, s.currentStreakRange ?? 'No active streak', { size: 10.5, fill: C.muted, anchor: 'middle' });
  body +=
    text(c3, y0 + 88, String(s.longestStreak), { size: 40, weight: 700, fill: P.pink, anchor: 'middle' }) +
    text(c3, y0 + 136, 'Longest Streak', { size: 14, anchor: 'middle' }) +
    text(c3, y0 + 166, s.longestStreakRange ?? '', { size: 10.5, fill: C.muted, anchor: 'middle' });
  ...
  tiles.forEach(([l, v, sub, color], i) => (body += tile(i * (tw + gap), ty, tw, 104, l, v, sub, color)));
  return { h: ty + 104, body };
}
...
export function momentum(contrib: Res<ContribData>): Block {
  ...
  series.forEach((s, i) => {
    ...
    if (y !== lastYear && s.date.slice(5, 7) <= '01') ticks += text(pt(i, 0)[0], top + ch + 26, y, { size: 9, fill: C.muted, anchor: 'middle' });
    lastYear = y;
  });
  ...
  const body =
    ...
    ticks +
    text(left + 12, top + 40, fmt(st.trailingYear), { size: 26, weight: 700, fill: P.pink }) +
    text(left + 12, top + 64, 'TRAILING 12 MONTHS', { size: 8.5, fill: C.muted, spacing: 0.6 }) +
    note(top + ch + 64, `${fmt(st.total)} since ${contrib.v.days[0].date}. Peak rolling year ${fmt(peak.value)} (${peak.date}).`);
  return { h: top + ch + 78, body };
}
...
export function heatmap(contrib: Res<ContribData>): Block {
  ...
  years.forEach((yr, yi) => {
    ...
    body += text(0, y + 14, `${yr} · ${fmt(contrib.v.totals[String(yr)] ?? 0)} contributions`, { size: 10, fill: ramp[4] });
    let prev = -1;
    weeks.forEach((wk, i) => {
      ...
      if (m !== prev) {
        body += text(LEFT + i * STEP, y + 38, MONTHS[m], { size: 9, fill: C.muted });
        prev = m;
      }
    });
    for (const d of [1, 3, 5]) body += text(0, y + 48 + d * STEP + 11, WEEKDAYS[d], { size: 9, fill: C.muted });
    weeks.forEach((wk, wi) =>
      wk.forEach((d, di) => {
        if (d) body += rect(LEFT + wi * STEP, y + 48 + di * STEP, CELL, CELL, ramp[d.level], 2.5);
      }),
    );
    y += 48 + 7 * STEP + 22;
  });
  body += text(INNER - 5 * 18 - 84, y + 11, 'Less', { size: 9, fill: C.muted });
  HEAT_RAMPS[0].forEach((f, i) => (body += rect(INNER - 5 * 18 - 30 + i * 18, y, 13, 13, f, 2.5)));
  body += text(INNER, y + 11, 'More', { size: 9, fill: C.muted, anchor: 'end' });
  return { h: y + 24, body };
}
...
export function rhythm(contrib: Res<ContribData>): Block {
  ...
  let body = meta(0, 14, 'By weekday') + meta(540, 14, 'By month');
  [1, 2, 3, 4, 5, 6, 0].forEach((d, i) => {
    const y = 36 + i * 36;
    body +=
      text(0, y + 15, WEEKDAYS[d], { size: 11, fill: WEEKDAY_COLORS[d] }) +
      rect(64, y, 290, 18, C.card2, 4) +
      rect(64, y, (week[d] / maxW) * 290, 18, WEEKDAY_COLORS[d], 4) +
      text(444, y + 15, fmt(week[d]), { size: 10.5, anchor: 'end', fill: C.muted });
  });
  const colW = 31;
  month.forEach((v, i) => {
    const h = Math.max(2, (v / maxM) * 180);
    const x = 540 + i * colW;
    body += rect(x, 36 + 190 - h, colW - 6, h, MONTH_COLORS[i], 3) + text(x + (colW - 6) / 2, 36 + 216, MONTHS[i][0], { size: 10, fill: MONTH_COLORS[i], anchor: 'middle' });
  });
  body += note(316, `${weekendShare(contrib.v.days).toFixed(0)}% of contributions land on weekends.`);
  return { h: 332, body };
}
...
export function repositories(res: Res<GhRepo[]>): Block {
  ...
  const cols = { name: 0, lang: 330, stars: 545, forks: 620, size: 735, upd: INNER };
  let body =
    meta(cols.name, 14, 'Repository') + meta(cols.lang, 14, 'Language') +
    meta(cols.stars, 14, 'Stars', 'end') + meta(cols.forks, 14, 'Forks', 'end') +
    meta(cols.size, 14, 'Size', 'end') + meta(cols.upd, 14, 'Updated', 'end');
  rows.forEach((r, i) => {
    const y = 32 + i * 42;
    body += line(0, y, INNER, y);
    body += text(cols.name, y + 27, trunc(r.name, 20), { size: 11.5, fill: P.gold });
    if (r.language) body += circle(cols.lang + 6, y + 20, 6, languageColor(r.language)) + text(cols.lang + 18, y + 27, trunc(r.language, 10), { size: 11 });
    else body += text(cols.lang, y + 27, '—', { size: 11, fill: C.muted });
    body += text(cols.stars, y + 27, fmt(r.stargazers_count), { size: 11, anchor: 'end', fill: P.gold });
    body += text(cols.forks, y + 27, fmt(r.forks_count), { size: 11, anchor: 'end', fill: P.blue });
    body += text(cols.size, y + 27, r.size >= 1024 ? `${(r.size / 1024).toFixed(1)} MB` : `${r.size} KB`, { size: 11, anchor: 'end' });
    body += text(cols.upd, y + 27, r.pushed_at.slice(0, 10), { size: 11, anchor: 'end', fill: C.muted });
  });
  body += note(32 + rows.length * 42 + 30, 'Ranked by stars. Per-repository commit counts need authentication.');
  return { h: 32 + rows.length * 42 + 42, body };
}
...
export function portfolio(res: Res<GhRepo[]>): Block {
  ...
  for (let y = new Date(start).getFullYear() + 1; y <= new Date(end).getFullYear(); y++) {
    ...
    body += line(tx, 0, tx, rows.length * rowH) + text(tx, rows.length * rowH + 24, y, { size: 9.5, fill: C.muted, anchor: 'middle' });
  }
  rows.forEach((r, i) => {
    ...
    body +=
      circle(8, y, 6, r.language ? languageColor(r.language) : C.grey) +
      text(26, y + 6, trunc(r.name, 18), { size: 11 }) +
      `<line x1="${a.toFixed(1)}" y1="${y}" x2="${b.toFixed(1)}" y2="${y}" stroke="${color}" stroke-width="3" stroke-linecap="round"/>` +
      circle(b, y, 3.5, color);
  });
  body += note(rows.length * rowH + 58, 'Bars run from creation to last push, coloured by language. Grey = archived.');
  return { h: rows.length * rowH + 70, body };
}
